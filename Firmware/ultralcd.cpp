...
//	st_synchronize();
    ...
    // abelsm2 unload filament using settings from variant
    // abelsm2 first extrude small amount to reduce tip size
    ...
    // abelsm2 then continue the normal prusa unload
...
//end of second measurement, now check for possible errors:
	...
	for(uint_least8_t i = 0; i < 2; i++){ //check if measured axis length corresponds to expected length
		...
		if (abs(measured_axis_length[0] - measured_axis_length[1]) > 5) { // abelsm2 loosen: check if difference between first and second measurement is low
			//loose pulleys
			const char *_error_1;
